...
#    expect_quadratic_time(opart_gaussian(rnorm(N), 1), data.sizes = 10^seq(1, 4, by = 0.5), max.seconds = 0.1)
#  })
#}
...
#    expect_loglinear_time(Fpop(rnorm(N), 1), data.sizes = 10^seq(1, 5, by = 0.5))
#  })
#}
...
#    expect_loglinear_time(gfpop(data = dataGenerator(as.integer(N), c(0.1, 0.2, 0.3, 0.4, 0.6, 0.8, 1), c(0, 0.5, 1, 1.5, 2, 2.5, 3), sigma = 1), mygraph = graph(penalty = 2*log(as.integer(N)), type = "isotonic"), type = "mean"), data.sizes = 10^seq(1, 4, by = 0.5))
#  })
#}
